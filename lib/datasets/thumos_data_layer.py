...
class TRNTHUMOSDataLayer(data.Dataset):
    def __init__(self, args, transform, phase='train'):
        ...
        self.camera_target_root = args.camera_target_root
        ...
        for session in self.sessions:
            target = np.load(osp.join(self.data_root, self.camera_target_root, session+'.npy'))   # shape:(num_frames, one_hot_vector)
            if self.sample_frames != 0:
                # Takes one frames every 'self.sample_frames' frames.  e.g. one frame every sixth
                #   (i.e. the step is self.sample_frames)
                # Furthermore, the frame to be taken is the one in the middle of the sequence
                #   (i.e. the start is self.sample_frames // 2)
    ...
    def __getitem__(self, index):
        ...
        for count in range(start, end):
            idx_frame = count
            if self.sample_frames != 0 :
                idx_frame = (idx_frame * self.sample_frames) + self.sample_frames//2
            ...
            frames[count-start] = frame.to(torch.float32)
