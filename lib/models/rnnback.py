...
class RNNBackmodel(nn.Module):
    def __init__(self, args):
        super(RNNBackmodel, self).__init__()
        self.hidden_size = args.hidden_size
        self.num_classes = args.num_classes
        self.steps = args.steps

        self.h_n = torch.zeros(args.batch_size, self.hidden_size)
        self.c_n = torch.zeros(args.batch_size, self.hidden_size) if self.model=='LSTM' else torch.zeros(1)
    ...
    def forward(self, x, mask):
        ...
        self.h_n = self.h_n.to(x.device, x.dtype)
        self.c_n = (self.c_n.to(x.device, x.dtype)) if self.model=='LSTM' else torch.zeros(1)
        ...
        for step in range(self.steps):
            ...
            self.h_n = self.h_n * mask[:, step]
            self.c_n = (self.c_n * mask[:, step]) if self.model=='LSTM' else torch.zeros(1)
            scores[:, step, :] = out
        return scores
