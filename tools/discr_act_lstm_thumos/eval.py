...
def main(args):
    ...
    if osp.isfile(args.checkpoint) and osp.isfile(args.checkpoint_act):
        checkpoint_discr = torch.load(args.checkpoint)          # checkpoint discr model
        checkpoint_act = torch.load(args.checkpoint_act)        # checkpoint act model
    else:
        raise(RuntimeError('Cannot find the checkpoint {} or {}'.format(args.checkpoint, args.checkpoint_act)))
    model = build_model(args).to(device)

    # load discr model and replace keys with the ones in DiscrActLSTM
    model_state_dict_discr = checkpoint_discr['model_state_dict']
    new_model_state_dict_discr = model_state_dict_discr.copy()
    for old_key in model_state_dict_discr.keys():
        if 'lstm' in old_key:
            new_key = old_key.replace('lstm', 'discr')
            new_model_state_dict_discr[new_key] = new_model_state_dict_discr.pop(old_key)
        elif 'classifier' in old_key:
            new_key = old_key.replace('classifier', 'discr_classifier')
            new_model_state_dict_discr[new_key] = new_model_state_dict_discr.pop(old_key)
        else:
            raise Exception('wrong model state dict rename')
    # load act model and replace keys with the ones in DiscrActLSTM
    model_state_dict_act = checkpoint_act['model_state_dict']
    new_model_state_dict_act = model_state_dict_act.copy()
    for old_key in model_state_dict_act.keys():
        if 'lstm' in old_key:
            new_key = old_key.replace('lstm', 'act')
            new_model_state_dict_act[new_key] = new_model_state_dict_act.pop(old_key)
        elif 'classifier' in old_key:
            new_key = old_key.replace('classifier', 'act_classifier')
            new_model_state_dict_act[new_key] = new_model_state_dict_act.pop(old_key)
        else:
            raise Exception('wrong model state dict rename')

    # now the two state dict are consistent with the DiscrActLSTM, so fuse them and load into DiscrActLSTM model
    new_model_state_dict_discr.update(new_model_state_dict_act)
    model.load_state_dict(new_model_state_dict_discr)
