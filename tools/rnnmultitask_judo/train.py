        ...
        for phase in args.phases:
            ...
            with torch.set_grad_enabled(training):
                for batch_idx, (camera_inputs, motion_inputs, targets, _) in enumerate(data_loaders[phase], start=1):
                    ...
                    # sum losses along all timesteps
                    act_back_loss = actback_criterion(act_back_scores[:, 0], act_back_targets[:, 0].max(axis=1)[1])
                    for step in range(1, camera_inputs.shape[1]):
                        act_back_loss += actback_criterion(act_back_scores[:, step], act_back_targets[:, step].max(axis=1)[1])
                    act_back_loss /= camera_inputs.shape[1]
