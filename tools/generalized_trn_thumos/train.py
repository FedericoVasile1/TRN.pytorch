...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        lr_sched.step(enc_losses['test'] / len(data_loaders['test'].dataset))
        ...
        for cls in range(args.num_classes):
            if cls == 0 or cls == 21:  # ignore background class and amibiguous class
                continue
            log += '| ' + args.class_index[cls] + ' AP: ' + str(enc_result['test']['AP'][args.class_index[cls]] * 100)[:4] + ' %'
        log += '| mAP: ' + str(enc_result['test']['mAP'] * 100)[:4] + ' %'
        log += '\n'
        ...
        log += ' [test] enc_loss: {:.5f}  enc_mAP: {:.4f}  dec_loss: {:.5f}  dec_mAP: {:.4f} |\n'
        ...
        log = str(log).format(epoch,
                              ...
                              dec_mAP['train'],
                              enc_losses['test'] / len(data_loaders['test'].dataset),
                              enc_mAP['test'],
                              dec_losses['test'] / len(data_loaders['test'].dataset),
                              dec_mAP['test'],
                              end - start)
        print(log)
        logger._write(log)

        if best_test_enc_mAP < enc_mAP['test']:
            best_test_enc_mAP = enc_mAP['test']
            epoch_best_test_enc_mAP = epoch

            # only the best validation map model is saved
            checkpoint_file = 'model-{}-features-{}.pth'.format(args.model, args.camera_feature)
            torch.save({
                'enc-test_mAP': best_test_enc_mAP,
                'epoch': epoch,
    ...
    log = '--- Best encoder test mAP is {:.1f} % obtained at epoch {} ---'.format(best_test_enc_mAP * 100, epoch_best_test_enc_mAP)
    print(log)
    logger._write(log)
