...
def main():
    ...
    with torch.set_grad_enabled(False):
        for folder in SUBFOLDERS:
            videos_dir = os.listdir(os.path.join(DATA_ROOT, VIDEO_FRAMES, folder))
            for dir in videos_dir:
                num_frames = len(os.listdir(os.path.join(DATA_ROOT, VIDEO_FRAMES, folder, dir)))
                ...
                for idx_frame in range(0, num_frames):
                    ...
                    frame = Image.open(os.path.join(DATA_ROOT, VIDEO_FRAMES, folder, dir, str(idx_frame+1)+'.jpg')).convert('RGB')
                ...
                np.save(os.path.join(DATA_ROOT, VIDEO_FEATURES, folder, str(dir)+'.npy'), feat_vects_video.numpy())
