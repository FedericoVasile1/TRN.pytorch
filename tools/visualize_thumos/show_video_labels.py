...
if __name__ == '__main__':
    ...
    parser.add_argument('--frames_dir', default='video_frames_24fps', type=str)
    parser.add_argument('--targets_dir', default='target_frames_24fps', type=str)
    ...
    # if each_class == True then a video is randomly sampled from each class
    parser.add_argument('--each_class', action='store_true')
    ...
    if args.video_name == '':
        ...
        args.each_class = False         # must be necessary False; only the video 'video_name' will be showed

    if args.each_class:
        for idx_class in range(1, args.num_classes):
            same_class_videos = []
            for video_name in videos_list:
                target = np.load(os.path.join(args.data_root, args.targets_dir, video_name+'.npy'))
                target = target.argmax(axis=1)
                if idx_class in target:
                    same_class_videos.append(video_name)
