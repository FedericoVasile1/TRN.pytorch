        ...
        for phase in args.phases:
            ...
            with torch.set_grad_enabled(training):
                for batch_idx, (inputs, targets) in enumerate(data_loaders[phase], start=1):
                    ...
                    mask = torch.zeros(targets.shape[0], targets.shape[1])
                    for step in args.steps:
                        targets_t = targets[:, step].argmax(dim=1)
                        targets_t[targets_t!=0] = 1
                        mask[:, step] = targets_t

                    scores = model(inputs, mask)            # scores.shape == (batch_size, steps, num_classes)
